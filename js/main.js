...
document.querySelectorAll('.quesions__button').forEach((el) => {
    el.addEventListener('click', () => {
        let content = el.nextElementSibling
        let item = el.parentElement
        let button = el
        let icon = el.childNodes[1]
        let minus = icon.childNodes[1]
        
        

        if(content.style.maxHeight) {
            document.querySelectorAll('.quesions__answer').forEach((el) => el.style.maxHeight = null)
            document.querySelectorAll('.quesions__item').forEach((el) => el.style.paddingBottom = null)
            document.querySelectorAll('.quesions__button').forEach((el) => el.classList.remove('quesions__button--acive'))
            document.querySelectorAll('.quesions__icon-minus').forEach((el) => el.classList.remove('quesions__icon-minus--hidden'))
                ...
                document.querySelectorAll('.quesions__button').forEach((el) => el.classList.remove('quesions__button--acive'))
                button.classList.add('quesions__button--acive')
                document.querySelectorAll('.quesions__icon-minus').forEach((el) => el.classList.remove('quesions__icon-minus--hidden'))
                minus.classList.add('quesions__icon-minus--hidden')
            }
    })
})
